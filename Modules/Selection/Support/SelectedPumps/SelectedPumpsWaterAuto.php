 ...
 */
final class SelectedPumpsWaterAuto implements Arrayable
{
    /**
     * Ctor.
                                                                ...
                                                                function (Collection $_collectors) use ($pump, $pumpsCount, $mainPumpsCount, $chassis, &$key, $rates) {
                                                                    return new ArrMapped(
                                                                        new ArrMapped(
                                                                            ...
                                                                            fn(string $controlSystemTypeId) => ControlSystem::allOrCached()
                                                                        ...
                                                                        function (?ControlSystem $controlSystem) use ($pump, $mainPumpsCount, $_collectors, $pumpsCount, $chassis, &$key, $rates) {
                                                                            return [
                                                                                'key' => $key++,
                                                                                'num' => $key,
                                                                                'created_at' => date_format(now(), 'd.m.Y'),
                                                                                'name' => "BPE PumpMaster WS "
                                                                                    . (ControlSystemType::allOrCached()->firstWhere('id', $controlSystem?->type_id)?->name ?? "?")
                                                                                    . " $pumpsCount"
                                                                                    . " " . $pump->series->name
                                                                                    . " " . $pump->name
                                                                                    . " Ду " . (($inputCollector = $_collectors->firstWhere('dn_pipes', $pump->dn_suction))?->dn_common ?? "?")
                                                                                    . " " . ($inputCollector?->material->description ?? ""),
