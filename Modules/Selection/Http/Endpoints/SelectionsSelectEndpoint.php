                ...
                TkJson::new(function () use ($request) {
                    ...
                    $selectedPumps = ArrMerged::new(
                        ...ArrMapped::new(
                            ...
                            function (Pump $pump) use (&$num, $request, $range, $rates) {
                                return ArrFiltered::new(
                                    ArrMapped::new(
                                        $request->main_pumps_counts ?? [$request->dp_work_scheme_id],
                                        function ($lastPumpPos) use (&$num, $pump, $request, $rates, $range) {
                                            $qEnd = (new PpQEnd($pump->performance(), $lastPumpPos))->asNumber();
                                            if ($request->flow < $qEnd) {
                                                $qStart = (new PpQStart($pump->performance(), $lastPumpPos))->asNumber();
                                                $intersectionPoint = new IntersectionPoint(
                                                    new EqFromPumpCoefficients(
                                                        $pump->coefficientsAt($lastPumpPos)
                                                    ),
                                                    $request->flow,
                                                    $request->head
                                                );
                                                ...
                                                if ($request->flow >= $qStart
                                                    ...
                                                    * (($request->deviation ?? 0) / 100)
                                                    ...
                                                    $isSingle = isset($request->reserve_pumps_count); // fixme
                                                    $pumpPrices = $pump->currentPrices($rates);
                                                    $pumpsCount = $lastPumpPos + ($request->reserve_pumps_count ?? 0);
                                                    $countTotal = $isSingle ? $pumpsCount : 1;
                                                    return [
                                                        'key' => $num++,
                                                        ...
                                                        'retail_price_total' => round($pumpPrices['simple'] * $countTotal, 2),
                                                        'discounted_price_total' => round($pumpPrices['discounted'] * $countTotal, 2),
                                                        ...
                                                        'head' => $request->head,
                                                        'flow' => $request->flow,
                                                        'main_pumps_count' => $isSingle ? $lastPumpPos : null,
